...
static inline int is_device_on(struct lsm303dlh_a_data *data)
{
	/* turn on the supplies if already off */
		if (data->mode == LSM303DLH_A_MODE_OFF&& data->regulator
	...
	if (data->device_status != LSM303DLH_A_DEVICE_ON) {
		WARN_ONCE(1, "%s: device is switched off, make it on using mode\n", __func__);
					...
					const struct i2c_device_id *id)
{
	...
	if (err)
		goto exit2;
int ret;
ret=i2c_smbus_read_byte_data(data->client, LSM303DLH_A_CHIP_ID);
printk(KERN_ERR "----------------------lsm303dlh_a_chip is %d------------------\n",ret);
